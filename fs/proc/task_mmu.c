...
static void seq_print_vma_name_sultan(struct seq_file *m, struct vm_area_struct *vma)
{
	const char __user *name = vma_get_anon_name(vma);
	struct mm_struct *mm = vma->vm_mm;
...
static int show_vma_header_prefix_sultan(struct seq_file *m, unsigned long start,
				  ...
				  unsigned long ino)
{
	...
	if (lyb_sultan_pid_header_shrink)
	{
		/* 
...
show_map_vma_sultan(struct seq_file *m, struct vm_area_struct *vma, int is_pid)
{
	struct mm_struct *mm = vma->vm_mm;
	struct file *file = vma->vm_file;
	vm_flags_t flags = vma->vm_flags;
	...
	if (show_vma_header_prefix_sultan(m, start, end, flags, pgoff, dev, ino))
		return;
...
{
	if (lyb_sultan_show_pid_map)
		show_map_vma_sultan(m, v, is_pid);
	else show_map_vma(m, v, is_pid);
	m_cache_vma(m, v);
	return 0;
}
...
#define SEQ_PUT_DEC(str, val) \
		seq_put_decimal_ull_width(m, str, (val) >> 10, 8)
static int show_smap_sultan(struct seq_file *m, void *v, int is_pid)
{
	struct proc_maps_private *priv = m->private;
	struct vm_area_struct *vma = v;
	struct mem_size_stats mss_stack;
	...
	if (!rollup_mode) {
		show_map_vma_sultan(m, vma, is_pid);
		if (vma_get_anon_name(vma)) {
			seq_puts(m, "Name:           ");
			seq_print_vma_name_sultan(m, vma);
		}
	} else if (last_vma) {
		show_vma_header_prefix_sultan(
			m, mss->first_vma_start, vma->vm_end, 0, 0, 0, 0);
		seq_puts(m, "[rollup]\n");
	} else {
		ret = SEQ_SKIP;
...
{
	if (lyb_sultan_show_pid_map)
		return show_smap_sultan(m, v, 1);
	else return show_smap(m, v, 1);
}

static int show_tid_smap(struct seq_file *m, void *v)
{
	if (lyb_sultan_show_pid_map)
		return show_smap_sultan(m, v, 0);
	else return show_smap(m, v, 0);
}
